      ...
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="text-center">
          ...
          <p className="text-xl text-slate-500 max-w-2xl mx-auto leading-relaxed">
            Discover premium audio experiences with our curated speaker collection
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
          {speakers.map((sp, index) => (
            <ProductCard
              ...
              product={sp}
              index={index}
              onClick={() => navigate(`/SpeakerDetail/${sp.id}`)}
            />
          ))}
        </div>
      </div>
...
const ProductCard = ({ product, index, onClick }) => {
  return (
    <div
      className="group bg-white/80 backdrop-blur-sm rounded-xl p-6 lg:p-8 shadow-lg hover:shadow-xl border border-slate-100 hover:border-indigo-200 transition-all duration-500 hover:-translate-y-2 hover:scale-[1.02] cursor-pointer flex flex-col h-full"
