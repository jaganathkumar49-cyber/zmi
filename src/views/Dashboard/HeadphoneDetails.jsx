...
const HeadphoneDetail = () => {
  ...
  const hp = headphones.find((h) => h.id === Number(id));
  ...
  const isInCart = cart.some(item => item.id === hp.id);

  const reviews = [
    {
      id: 1, user: "Rahul M.", rating: 5, date: "2 days ago",
      comment: "Unbelievable sound quality! The ANC is next level.", verified: true
    },
    {
      id: 2, user: "Priya K.", rating: 4, date: "1 week ago",
      comment: "Comfortable for long sessions. Battery life exceeds expectations.", verified: true
    }
  ];
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white/60 backdrop-blur-sm rounded-xl p-6 border border-slate-100 shadow-lg grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="flex flex-col items-center p-3 group hover:scale-105 transition-all">
                  ...
                  <span className="text-sm font-medium text-slate-700 text-center">Noise Cancel</span>
                </div>
                <div className="flex flex-col items-center p-3 group hover:scale-105 transition-all">
                  <Bluetooth className="w-10 h-10 text-purple-500 mb-2 group-hover:scale-110" />
                  <span className="text-sm font-medium text-slate-700 text-center">Bluetooth 5.0</span>
                </div>
                <div className="flex flex-col items-center p-3 group hover:scale-105 transition-all">
                  <Volume2 className="w-10 h-10 text-indigo-600 mb-2 group-hover:scale-110" />
                  <span className="text-sm font-medium text-slate-700 text-center">Premium Sound</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8 lg:pt-4">
              ...
              <div className="bg-white/60 backdrop-blur-sm rounded-xl p-8 border border-slate-100 shadow-lg">
                ...
                <p className="text-lg text-slate-600 leading-relaxed">{hp.desc}</p>
              </div>

              {/* Action Buttons */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <button
                  onClick={() => addToCart(hp)}
                  disabled={isInCart}
          ...
          {showReviews && (
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-8 lg:p-12 border border-slate-100 shadow-xl">
              <h2 className="text-3xl font-bold text-slate-900 mb-8 flex items-center gap-3">
                Customer Reviews
                <span className="text-sm bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full font-semibold">4.7 ★</span>
              </h2>
